...
function upsertProperty(prop) {
    // Accept both JSON:API format ({ attributes: { name, platform } })
    // and flat format ({ id, name }) as returned by getSelectedProperties()
    const name = prop.attributes?.name ?? prop.name;
    const platform = prop.attributes?.platform ?? prop.platform ?? null;
    db.prepare(`INSERT OR REPLACE INTO properties
      ...
      VALUES (@id, @name, @platform, @attributes_json, @cached_at)`).run({
        ...
        name,
        platform,
        attributes_json: JSON.stringify(prop.attributes ?? { name, platform }),
        cached_at: now()
    });
}
...
function getRulesForLibrary(libraryId) {
    ...
    const rcStmt = db.prepare(
        'SELECT * FROM rule_components WHERE rule_id = ? ORDER BY sort_order'
    );
    ...
    return rows.map(row => ({
        ...
        rule_components: rcStmt.all(row.id).map(rc => ({
            id: rc.id,
            type: 'rule_components',
            attributes: JSON.parse(rc.attributes_json),
            relationships: { origin: { data: { id: rc.origin_id } } }
        }))
    }));
}
